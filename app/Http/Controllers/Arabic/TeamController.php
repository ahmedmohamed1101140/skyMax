...
class TeamController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('site_ar.team', compact('countries',
            'user',
            'categories',
            'states',
            'bool',
            ...
            if ($parent) {
                ...
                elseif ($children->count() == 1){
                    if($data['position'] == 1 && substr($children->first()->code_count,-1) == '1' ){
                        ...
                        session()->flash('error','لا يمكن اضافه مستخدم فى هذا المكان');
                        return redirect()->back();
                    }
                }
                else{
                    session()->flash('error',"هذا العميل لديه العدد الكافى من المستخدمين حاول مع عميل اخر");
                    return redirect()->back();
                }
                ...
                session()->flash('error',"خطأ فى الحصول على الاب");
                return redirect()->back();
            }
        }
        else{
            session()->flash('error',"ليس لديك الحساب الكافى لإضافه مستخدمين جدد");
            return redirect()->back();
        }
    }
...
//                session()->flash('message','Client Tree Found');
                return view('site_ar.team',compact('user','bool','countries','categories', 'states','children','client'));
            }
            else{
                session()->flash('error',"الابناء الخاصين فقط");
                return redirect()->back();
            }
        }
        else{
            session()->flash('message','Client Not Found');
            return view('site_ar.team',compact('user','countries','categories', 'states','children','client'));
    ...
    public function find_all_parents($user){
        ...
        for ($i=1 ; $i<strlen($user->code_count) ; $i++){
            ...
            if($client){
                ...
                session()->flash('error',"حدث خطأ حاول مره اخرى");
                return redirect()->back();
            }
        }
    }

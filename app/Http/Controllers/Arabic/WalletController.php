...
class WalletController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('site_ar.wallet', compact('e_pins','e_pins1','e_moneys1','e_moneys', 'countries','states','count_direct','count_binary','count_store'));
    }
    ...
    public function filter(Request $request)
    {
        ...
        return view('site_ar.wallet', compact(
            'binary_commission',
            'direct_commission',
            'store_commission',
            'e_pins',
    ...
    public function transfer(Request $request)
    {
        ...
        if($user == null){
            session()->flash('error',"لم ننتمكن من إيجاد هذا المستخدم");
        ...
        if($this->check_code_count($auth_user,$user)){
            ...
            if($user && ($pincode == $auth_user->pincode) && $user->id != $auth_user->id){
                if ($request->e_type_id == 0) {
                    if ($auth_user->emoney >= (int)$request->value) {
                        ...
                        $user->save();
                        session()->flash('message',"تم التحويل بنجاح");
                        return redirect()->back();
                    }
                    else{
                        session()->flash('error',"ليس لديك الحساب الكافى");
                        return redirect()->back();
                    }
                }
            ...
            session()->flash('error',"يمكنك التحويل فقط لمن هم من نفس شبكتك");
            return redirect()->back();
        }
    }

...
class ProfileController extends Controller
{
    ...
    public function index()
    {
        //
        $client = User::find(1);
        return view('site_ar.team',['user' => $client]);
    }
    ...
    public function update_profile(Request $request){
        ...
        if($request->old_password && $request->new_password){
            if(Hash::check($request->old_password, auth()->user()->password)){
                ...
                session()->flash('error','الرقم السرى غير صحيح');
        ...
        session()->flash('message','تم تغير الرقم السرى');
        return redirect()->back();
    }
    ...
    public function renew_account(Request $request){
        ...
        if(auth()->user()->epin >= $main_data->price_activeaccount){
            ...
            session()->flash('message','تمم تجديد الحساب');
            return redirect()->back();

        }
        else{
            session()->flash('error','عفوا ليس لديك حساب كاف');
            return redirect()->back();
        }

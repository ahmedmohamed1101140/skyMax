    ...
    public function index(Request $request){
        ...
        if($request->password && $user){
            if($request->password != auth()->user()->pincode){
                session()->flash('error','Wrong Inside Password');
                return redirect()->back();
            }
        }
        ...
        if(($user->activation == 0 && $product->type == 2) || ($user->activation == 1 && $product->type == 1)|| $product->amount < $request->amount){
            session()->flash('error','Sorry cant Buy This Item');
            return redirect()->back();
        }
        ...
        if($product->type == 1){
            if($product->discount > 0){
                if($user->emoney < ($product->discount * $request->amount) && $user->epin < ($product->shipping_phease*$request->amount)){
                    session()->flash('error','Sorry Not Enough Money for this Qualified Product');
                    return redirect()->back();
                }
            }

            //check if the user wallet have the amount to buy the product
            if (($user->emoney >= ($product->price*$request->amount) && $user->epin >= ($product->shipping_phease*$request->amount)) || ($user->emoney >= ($product->discount * $request->amount) && $user->epin >= ($product->shipping_phease*$request->amount) && $product->discount > 0)) {
                ...
                session()->flash('message','Congratulation Qualified Item/s Added');
                return redirect()->back();
        ...
        else if($product->type == 2)
        {
            if($product->discount > 0){
                if($user->epin < ($product->discount * $request->amount)){
                    session()->flash('error','Sorry Not Enough Money for this Premium Product');
                    return redirect()->back();
                }
            }
            //check if the user wallet have the amount to buy the product
            if (($user->epin >= ($product->price*$request->amount))|| ($user->epin>= ($product->discount * $request->amount))) {
                ...
                session()->flash('message','Congratulation Premium Item/s Added');
                return redirect()->back();
            }
            else{
                session()->flash('message','Sorry Not Enough Money for this Qualified Product');
                return redirect()->back();
            }
        }

        else {
            session()->flash('error','error happen');
            return redirect()->back();
    ...
    public function order_video(Request $request){
        ...
        if($product->type == 1){
            ...
            if (($user->emoney >= $product->price && $user->epin >= $product->shipping_phease) || ($user->emoney >= $product->discount && $user->epin >= $product->shipping_phease)) {
                ...
                $this->find_user_parents($user);
                session()->flash('message','Congratulation Qualified Video Item/s Added');
                return redirect()->back();
            }
            else{
                session()->flash('error','Sorry Not Enough Money for this Qualified Video Product');
                return redirect()->back();
            }
        }
        ...
        else if($product->type == 2)
        {
            ...
            if (($user->epin >= $product->price)|| ($user->epin >= $product->discount )) {
                ...
                session()->flash('error','Congratulation Premium Item/s Added');
                return redirect()->back();
            }
            else{
                session()->flash('error','Sorry Not Enough Money for this Qualified Video Product');

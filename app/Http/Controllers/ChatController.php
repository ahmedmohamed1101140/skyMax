...
class ChatController extends Controller
{
    ...
    public function index(){
        ...
        foreach ($user->friends as $friend){
            if($friend->friend_id == -1){
                return view('site.friends', compact('countries','states'));
            }
        }
    ...
    public function add_friend(Request $request){
        if($request->user() == auth()->user()->usercode){
            session()->flash('error',"you use your own user code Please try another");
            return redirect()->back();
        }
        $user = Client::all()->where('usercode','=',$request->usercode)->first();
        foreach (auth()->user()->friends as $friend){
            if($friend->friend_id == $user->id){
                session()->flash('error','You already Have this friend in your friend List');
                return redirect()->back();
            }
        }
        $conv_id = Carbon::now()->timestamp;
        ...
        session()->flash('message','Your Friend Add Send Him a Message');
        return redirect()->back();
    ...
    public function show_message($id){
        ...
        return view('site.chat',compact('messages','countries','states'));
    }
